...
OUTPUT_DIM = 64*256*3 # Number of pixels in each iamge
...
def GoodGenerator(n_samples, noise=None, dim=DIM, nonlinearity=tf.nn.relu):
    if noise is None:
        noise = tf.random_normal([n_samples, 512])
        print noise.shape
    output = lib.ops.linear.Linear('Generator.Input', 512, 8*8*8*dim, noise)
    print output.shape
    output = tf.reshape(output, [-1, 8*dim, 4, 16])
    print output.shape
...
def GoodDiscriminator(inputs, dim=DIM):
    print 'output shape'
    output = tf.reshape(inputs, [-1, 3, 64, 256])
    print output.shape
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    all_real_data_conv = tf.placeholder(tf.int32, shape=[BATCH_SIZE, 3, 64, 256])
    ...
    fixed_noise = tf.constant(np.random.normal(size=(BATCH_SIZE, 512)).astype('float32'))
    ...
    def generate_image(iteration):
        ...
        lib.save_images.save_images(samples.reshape((BATCH_SIZE, 3, 64, 256)), 'samples_{}.png'.format(iteration))
    ...
    lib.save_images.save_images(_x_r.reshape((BATCH_SIZE/N_GPUS, 3, 64, 256)), 'samples_groundtruth.png')
    ...
    for iteration in xrange(ITERS):
        ...
        saver.save(session,'checkpoints/mnist.ckpt',global_step=iteration)
